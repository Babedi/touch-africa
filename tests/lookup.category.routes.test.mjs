/**
 * Comprehensive E2E Test: Lookup Category Module Routes
 *
 * This script tests all CRUD operations for the lookupCategory module:
 ...
 * - GET /internal/lookupCategory/search
...
async function testGetAllLookupCategories() {
  console.log("\n🧪 Testing GET ALL Lookup Categories...");

  const result = await makeRequest("GET", "/internal/lookupCategory/list");

  if (result.success && result.data.success) {
    console.log(
      `✅ GET ALL Success: ${result.data.data.length} lookup categories found`
    );
    return true;
  } else {
...
async function testGetLookupCategoryById(id) {
  console.log(`\n🧪 Testing GET Lookup Category by ID: ${id}`);

  const result = await makeRequest("GET", `/internal/lookupCategory/${id}`);

  if (result.success && result.data.success) {
    console.log("✅ GET BY ID Success:", result.data.data.category);
    return true;
  } else {
...
async function testUpdateLookupCategory(id) {
  console.log(`\n🧪 Testing UPDATE Lookup Category: ${id}`);
  ...
  if (result.success && result.data.success) {
    console.log("✅ UPDATE Success:", result.data.data.category);
    return true;
  } else {
...
async function testSearchLookupCategories() {
  console.log("\n🧪 Testing SEARCH Lookup Categories...");

  const result = await makeRequest(
    "GET",
    "/internal/lookupCategory/search?category=Emergency"
  );
...
async function testCheckLookupCategoryExists(id) {
...
async function testDeleteLookupCategory(id) {
  console.log(`\n🧪 Testing DELETE Lookup Category: ${id}`);

  const result = await makeRequest("DELETE", `/internal/lookupCategory/${id}`);
...
async function runAllTests() {
  console.log("🚀 Starting Lookup Category Module E2E Tests");
  ...
  // Test 3: Create lookup category
  totalTests++;
  createdId = await testCreateLookupCategory();
  if (createdId) testsPassed++;

  if (createdId) {
    // Test 4: Get all lookup categories
    totalTests++;
    if (await testGetAllLookupCategories()) testsPassed++;

    // Test 5: Get lookup category by ID
    totalTests++;
    if (await testGetLookupCategoryById(createdId)) testsPassed++;

    // Test 6: Check if lookup category exists
    totalTests++;
    if (await testCheckLookupCategoryExists(createdId)) testsPassed++;

    // Test 7: Search lookup categories
    totalTests++;
    if (await testSearchLookupCategories()) testsPassed++;

    // Test 8: Update lookup category
    totalTests++;
    if (await testUpdateLookupCategory(createdId)) testsPassed++;

    // Test 9: Delete lookup category
    totalTests++;
    if (await testDeleteLookupCategory(createdId)) testsPassed++;

    // Test 10: Verify deletion
    totalTests++;
    const result = await makeRequest(
      "GET",
      `/internal/lookupCategory/${createdId}`
    );
    if (!result.success && result.status === 404) {
      console.log(
        "\n✅ VERIFY DELETION Success: Lookup category not found after deletion"
      );
      testsPassed++;
    } else {
      console.error(
        "\n❌ VERIFY DELETION Failed: Lookup category still exists"
      );
    }
  }
  ...
  if (testsPassed === totalTests) {
    console.log(
      "\n🎉 ALL TESTS PASSED! Lookup Category module is working correctly."
    );
  } else {
