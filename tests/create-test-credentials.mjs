...
/**
 * Get authentication token from token.txt
 */
function getAuthToken() {
  try {
    const tokenPath = join(__dirname, "token.txt");
...
async function createTenantAdmin(tenantId) {
  ...
  try {
    const response = await apiRequest(`/external/tenantAdmin/${tenantId}`, {
      method: "POST",
      headers: {
...
async function createTenantUser(tenantId) {
  ...
  const tenantUserData = {
    ...
    // Add password for login testing
    accessDetails: {
  ...
  try {
    const response = await apiRequest("/external/tenantUser", {
      method: "POST",
      headers: {
        "x-tenant-id": tenantId,
      },
...
async function main() {
  ...
  try {
    // Create tenant admin
    const tenantAdmin = await createTenantAdmin(tenantId);

    // Create tenant user
    const tenantUser = await createTenantUser(tenantId);
    ...
    console.log("   Click the 🏢 icon for Tenant Admin login");
    console.log("   Click the 👤 icon for Tenant User login");
  } catch (error) {
