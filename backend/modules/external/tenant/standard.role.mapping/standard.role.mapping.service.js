import {
  createStandardRoleMapping,
  getStandardRoleMappingById,
  updateStandardRoleMappingById,
  deleteStandardRoleMappingById,
  getAllStandardRoleMappings,
  getStandardRoleMappingByName,
} from "./standard.role.mapping.firestore.js";
import { newStandardRoleMappingId } from "./standard.role.mapping.validation.js";
...
/**
 * Create a new standard role mapping with metadata
 */
export const createStandardRoleMappingService = async (
  roleMappingData,
  actor
) => {
  const mappingId = newStandardRoleMappingId();
  ...
  return await createStandardRoleMapping(roleMappingWithMetadata);
};

/**
 * Get role mapping by ID
 */
export const getStandardRoleMappingByIdService = async (mappingId) => {
  return await getStandardRoleMappingById(mappingId);
};

/**
 * Get role mapping by name
 */
export const getStandardRoleMappingByNameService = async (roleName) => {
  return await getStandardRoleMappingByName(roleName);
};

/**
 * Update role mapping by ID with metadata
 */
export const updateStandardRoleMappingByIdService = async (
  mappingId,
  updateData,
  actor
) => {
  ...
  const result = await updateStandardRoleMappingById(
    mappingId,
    dataWithMetadata
  );
  ...
  if (updateData.roleCode) {
    const mapping = await getStandardRoleMappingById(mappingId);
    if (mapping) {
 ...
 */
export const deleteStandardRoleMappingByIdService = async (mappingId) => {
  // Get the mapping first to update config
  const mapping = await getStandardRoleMappingById(mappingId);

  const result = await deleteStandardRoleMappingById(mappingId);
 ...
 */
export const getAllStandardRoleMappingsService = async () => {
  const dbMappings = await getAllStandardRoleMappings();
 ...
 */
export const syncRoleMappingsService = async (actor) => {
  const configMappings = roleMappingConfig.getAllMappings();
  const dbMappings = await getAllStandardRoleMappings();
    ...
    ([roleName, roleCode]) => {
      ...
      if (existingMapping) {
        ...
        if (existingMapping.roleCode !== roleCode) {
          return updateStandardRoleMappingByIdService(
        ...
        // Create new mapping
        return createRoleMappingService({ roleName, roleCode }, actor);
      }
  ...
  return await getAllStandardRoleMappingsService();
};

/**
 * Get role code by role name
 */
export const getRoleCodeByNameService = async (roleName) => {
  const mapping = await getStandardRoleMappingByName(roleName);
 ...
 */
export const roleMappingExistsService = async (roleName) => {
  const mapping = await getStandardRoleMappingByName(roleName);
  return !!mapping;
};
...
export const bulkCreateRoleMappingsService = async (
  ...
  for (const mappingData of mappingsData) {
    try {
      const existing = await getStandardRoleMappingByName(mappingData.roleName);
      ...
      if (existing && overwrite) {
        const updated = await updateStandardRoleMappingByIdService(
          existing.mappingId,
          mappingData,
          actor
        );
        ...
        const created = await createRoleMappingService(mappingData, actor);
        results.push({
          roleName: mappingData.roleName,
          status: "created",
          data: created,
 ...
 */
export async function listRoleMappingsService(queryParams = {}) {
  try {
    // Get all role mappings from Firestore
    const roleMappings = await getAllStandardRoleMappings();
 ...
 */
export async function searchRoleMappingsService(queryParams = {}) {
  try {
      ...
      case "delete":
        for (const [index, item] of data.entries()) {
          try {
            ...
            await deleteStandardRoleMappingByIdService(
              mappingId,
              "bulk_operation"
            );
            results.successful++;
 ...
 */
export async function getRoleMappingsStatsService() {
  try {
    const roleMappings = await getAllStandardRoleMappings();
