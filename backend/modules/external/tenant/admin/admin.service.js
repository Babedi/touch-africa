...
import {
  createExternalAdmin,
  getExternalAdminById,
  updateExternalAdminById,
  deleteExternalAdminById,
  activateExternalAdminById,
  deactivateExternalAdminById,
  getAllExternalAdmins,
  getExternalAdminByEmail,
  getAdminCollectionPath,
} from "./admin.firestore.js";
import { newExternalAdminId } from "./admin.validation.js";
...
// Create External admin service
export const createExternalAdminService = async (
  adminData,
  actor,
  tenantId
) => {
  ...
  const existingAdminByEmail = await getExternalAdminByEmail(
  ...
  const existingAdmins = await getAllExternalAdmins(tenantId);
  ...
  if (isFirstAdmin) {
    // Assign explicit highest-privilege External role for first admin
    adminWithDefaults.roles = ["ExTERNAL_ROOT_ADMIN"];
  }

  // Create in Firestore
  const createdAdmin = await createExternalAdmin(adminWithDefaults, tenantId);
...
// Get External admin by ID service
export const getExternalAdminByIdService = async (id, tenantId) => {
  const admin = await getExternalAdminById(id, tenantId);
  if (!admin) {
    return null;
  }
...
// Update External admin service
export const updateExternalAdminByIdService = async (
  id,
  updateData,
  actor,
  tenantId
) => {
  const existingAdmin = await getExternalAdminById(id, tenantId);
  if (!existingAdmin) {
  ...
  if (
    existingAdmin.roles &&
    existingAdmin.roles.includes("ExTERNAL_ROOT_ADMIN") &&
    actor !== id
  ) {
  ...
  await updateExternalAdminById(id, updatedAdmin, tenantId);
...
// Delete External admin service
export const deleteExternalAdminByIdService = async (id, actor, tenantId) => {
  const existingAdmin = await getExternalAdminById(id, tenantId);
  if (!existingAdmin) {
    throw new Error("Admin not found");
  }
  ...
  await deleteExternalAdminById(id, tenantId);
  return true;
};

// List all External admins service
export const listExternalAdminsService = async (queryParams = {}, tenantId) => {
  try {
    // Build Firestore query (use collection path string for compatibility)
    const collectionPath = getAdminCollectionPath(tenantId);
    const firestoreQuery = buildFirestoreQuery(collectionPath, queryParams);
    ...
    console.error("Error in listExternalAdminsService:", error);
    throw error;
  }
};

// Search External admins service
export const searchExternalAdminsService = async (
  queryParams = {},
  tenantId
) => {
  try {
    ...
    return await listExternalAdminsService(searchParams, tenantId);
  } catch (error) {
    console.error("Error in searchExternalAdminsService:", error);
    throw error;
  }
};

// Bulk operations service
export const bulkExternalAdminsService = async (
  operation,
  data,
  filters,
  actor,
  ...
  try {
    ...
    switch (operation) {
      case "create":
        ...
        for (const adminData of data) {
          try {
            await createExternalAdminService(adminData, actor, tenantId);
            results.success++;
          } catch (error) {
            results.failed++;
      ...
      case "update":
        ...
        for (const { id, ...updateData } of data) {
          try {
            await updateExternalAdminByIdService(
              id,
              updateData,
              actor,
              tenantId
      ...
      case "activate":
        ...
        for (const id of idsToActivate) {
          try {
            await activateExternalAdminService(id, actor, tenantId);
            results.success++;
          } catch (error) {
            results.failed++;
            results.errors.push({ id, error: error.message });
      ...
      case "deactivate":
        ...
        for (const id of idsToDeactivate) {
          try {
            await deactivateExternalAdminService(id, actor, tenantId);
            results.success++;
          } catch (error) {
            results.failed++;
            results.errors.push({ id, error: error.message });
    ...
    console.error("Error in bulkExternalAdminsService:", error);
    throw error;
  }
};

// Export External admins service
export const exportExternalAdminsService = async (
  queryParams = {},
  format = "csv",
  tenantId
) => {
  try {
    ...
    const result = await listExternalAdminsService(exportParams, tenantId);
    ...
    console.error("Error in exportExternalAdminsService:", error);
    throw error;
  }
};

// Get External admins statistics service
export const getExternalAdminsStatsService = async (
  queryParams = {},
  tenantId
) => {
  try {
    const result = await listExternalAdminsService(
      {
        ...queryParams,
        limit: 10000,
      },
    ...
    console.error("Error in getExternalAdminsStatsService:", error);
    throw error;
  }
};

// Login service
export const loginExternalAdminService = async (email, password, tenantId) => {
  let admin = await getExternalAdminByEmail(email, tenantId);
  ...
  await updateExternalAdminById(admin.id, admin, tenantId);
...
export const activateExternalAdminService = async (id, actor, tenantId) => {
  const change = {
    by: actor || "system",
    when: new Date().toISOString(),
    action: "activated",
  ...
  const activatedAdmin = await activateExternalAdminById(id, change, tenantId);
...
export const deactivateExternalAdminService = async (id, actor, tenantId) => {
  const existingAdmin = await getExternalAdminById(id, tenantId);
  if (!existingAdmin) {
    throw new Error("Admin not found");
  }
  ...
  const deactivatedAdmin = await deactivateExternalAdminById(
    id,
    change,
    tenantId
  );
