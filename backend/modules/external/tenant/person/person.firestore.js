...
/**
 * Generate dynamic people collection path based on tenant ID
 * @param {string} tenantId - The tenant identifier
 * @returns {string} The collection path for the tenant's people
 */
export function getPeopleCollectionPath(tenantId) {
  if (!tenantId) {
    throw new Error("Tenant ID is required for people collection path");
  }
  return `touchAfrica/${serviceId}/tenants/${tenantId}/people`;
}

/**
 * Generate dynamic counters path based on tenant ID
 * @param {string} tenantId - The tenant identifier
 * @returns {string} The counter document path for the tenant
 */
export function getCountersPath(tenantId) {
  if (!tenantId) {
    throw new Error("Tenant ID is required for counters path");
  }
  return `touchAfrica/${serviceId}/tenants/${tenantId}/counters/person`;
}

/**
 * Get reference to people collection for a specific tenant
 * @param {string} tenantId - The tenant identifier
 * @returns {FirebaseFirestore.CollectionReference}
 */
function peopleCol(tenantId) {
  return db.collection(getPeopleCollectionPath(tenantId));
}

/**
 * Get reference to person counters document for a specific tenant
 * @param {string} tenantId - The tenant identifier
 * @returns {FirebaseFirestore.DocumentReference}
 */
function countersDoc(tenantId) {
  return db.doc(getCountersPath(tenantId));
}

/**
 * Generate next person sequence number for a specific tenant
 * @param {string} tenantId - The tenant identifier
 * @returns {Promise<number>} Next sequence number
 */
async function nextPersonSequence(tenantId) {
  const ref = countersDoc(tenantId);
  let next;
...
/**
 * Create a new person record for a specific tenant
 * @param {Object} model - Person data model
 * @param {string} tenantId - The tenant identifier
 * @returns {Promise<Object>} Created person with ID
 */
export async function createPerson(model, tenantId) {
  try {
    // Check for duplicate SA ID number if provided
    if (model.idNumber) {
      const duplicateQuery = await peopleCol(tenantId)
    ...
    const docRef = peopleCol(tenantId).doc(model.id);
...
/**
 * Get person by ID for a specific tenant
 * @param {string} id - Person ID
 * @param {string} tenantId - The tenant identifier
 * @returns {Promise<Object|null>} Person data or null if not found
 */
export async function getPersonById(id, tenantId) {
  try {
    const doc = await peopleCol(tenantId).doc(id).get();
    if (!doc.exists) {
      return null;
    }
...
/**
 * Update person by ID for a specific tenant
 * @param {string} id - Person ID
 * @param {Object} data - Updated person data
 * @param {string} tenantId - The tenant identifier
 * @returns {Promise<Object>} Updated person data
 */
export async function updatePersonById(id, data, tenantId) {
  try {
    const docRef = peopleCol(tenantId).doc(id);
...
/**
 * Delete person by ID for a specific tenant
 * @param {string} id - Person ID
 * @param {string} tenantId - The tenant identifier
 * @returns {Promise<boolean>} True if deleted successfully
 */
export async function deletePersonById(id, tenantId) {
  try {
    const docRef = peopleCol(tenantId).doc(id);
...
/**
 * Get all persons with pagination for a specific tenant
 ...
 */
export async function getAllPersons(options = {}, tenantId) {
  try {
    const {
      limit = 100,
      startAfter = null,
    ...
    let query = peopleCol(tenantId)
      .orderBy(orderBy, orderDirection)
      .limit(limit);

    // Add pagination if startAfter provided
    if (startAfter) {
      const startAfterDoc = await peopleCol(tenantId).doc(startAfter).get();
    ...
    const totalSnapshot = await peopleCol(tenantId).get();
...
/**
 * Search persons by various criteria for a specific tenant
 ...
 */
export async function searchPersons(criteria, tenantId) {
  try {
    const results = [];

    // Search by SA ID number
    if (criteria.idNumber) {
      const snapshot = await peopleCol(tenantId)
