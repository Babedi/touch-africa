 ...
 */
export async function createPersonRecord(tenantId, data, actor) {
  try {
    ...
    const created = await createPerson(personModel, tenantId);
 ...
 */
export async function updatePersonRecord(tenantId, id, data, actor) {
  try {
    // Get existing person to check current data FIRST
    const existing = await getPersonById(tenantId, id);
    if (!existing) {
    ...
    const updated = await updatePersonById(tenantId, id, sanitizedData);
 ...
 */
export async function deletePersonRecord(tenantId, id, actor) {
  try {
    console.log(`🗑️ Deleting person: ${id} by ${actor}`);
    return await deletePersonById(tenantId, id);
  } catch (error) {
 ...
 */
export async function getAllPersonRecords(tenantId, queryParams = {}, actor) {
  try {
    ...
    const firestoreQuery = buildFirestoreQuery(
      getPeopleCollectionPath(tenantId),
      queryParams
    );
 ...
 */
export async function searchPersonRecords(tenantId, queryParams = {}, actor) {
  try {
    ...
    return await getAllPersonRecords(tenantId, nextParams, actor);
  } catch (error) {
 ...
 */
export async function bulkPersonRecords(
  tenantId,
  operation,
  data,
  filters,
    ...
    switch (operation) {
      case "create":
        ...
        for (const personData of data) {
          try {
            await createPersonRecord(tenantId, personData, actor);
            results.success++;
          } catch (error) {
            results.failed++;
      ...
      case "update":
        ...
        for (const { id, ...updateData } of data) {
          try {
            await updatePersonRecord(tenantId, id, updateData, actor);
            results.success++;
          } catch (error) {
            results.failed++;
      ...
      case "delete":
        ...
        for (const id of idsToDelete) {
          try {
            await deletePersonRecord(tenantId, id, actor);
            results.success++;
          } catch (error) {
            results.failed++;
            results.errors.push({ id, error: error.message });
...
export async function exportPersonRecords(
  ...
  try {
    ...
    const result = await getAllPersonRecords(tenantId, exportParams, actor);
 ...
 */
export async function getPersonStatistics(tenantId, queryParams = {}, actor) {
  try {
    console.log(`📊 Getting person statistics for ${actor}`);

    // For stats, we don't need sorting - just get all records
    const firestoreQuery = db.collection(getPeopleCollectionPath(tenantId));
    const snapshot = await firestoreQuery.get();
    ...
    persons.forEach((person) => {
      ...
      const gender =
        person.demographics?.gender?.toLowerCase() || "unspecified";
      if (stats.gender[gender] !== undefined) {
        stats.gender[gender]++;
      } else {
