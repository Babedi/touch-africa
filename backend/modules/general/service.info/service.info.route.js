...
router.patch(
  "/general/service-info",
  (req, res, next) => {
    console.log("🔍 Service Info Patch Route - Before auth middleware");
    console.log("  Body type:", typeof req.body);
    console.log("  Body keys:", Object.keys(req.body || {}));
    console.log("  Headers:", req.headers);
    next();
  ...
  authenticateJWT,
  (req, res, next) => {
    console.log("🔍 Service Info Patch Route - After auth, before authorize");
    next();
  },
  checkPermissions("service.read"),
  (req, res, next) => {
    console.log(
      "🔍 Service Info Patch Route - After authorize, before controller"
    );
    next();
  },
  patchServiceInfoHandler
...
router.get("/general/service-info/news", async (req, res) => {
  try {
    console.log(
      "📰 Loading news from Firebase path: /touchAfrica/southAfrica/news"
    );
    ...
    if (snapshot.empty) {
      ...
      const sampleNews = [
        {
          id: "SAMPLE_001",
          title: "🚨 Welcome to touchAfrica™ News System",
    ...
    snapshot.forEach((doc) => {
      ...
      newsItems.push({
        ...
        url: data.url || null,
        author: data.author || "touchAfrica™",
      });
    });
...
router.get("/general/tenants", async (req, res) => {
  try {
    console.log(
      "🏘️ Loading tenants from Firebase path: /touchAfrica/southAfrica/tenants"
    );
...
router.get("/general/service-info/features", async (req, res) => {
  try {
    console.log(
      "🔧 Loading features from Firebase path: /touchAfrica/southAfrica"
    );
