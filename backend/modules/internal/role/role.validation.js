...
export const newInternalRoleId = () => `IROLE${Date.now()}`;
...
export const InternalRoleUpdateSchema = InternalRoleSchema.partial().omit({
  roleCode: true,
  isSystem: true,
});

// For PATCH/partial updates it's common to allow shorter description if provided; however
// the above InternalRoleUpdateSchema already makes fields optional. The backend will accept
// provided description as long as it respects max length; preserving roleName/roleCode anyway.

// Check if valid role ID
export function isValidInternalRoleId(id) {
  // Allow standard IROLE format or special system role IDs
  return /^IROLE\d+$/.test(id) || /^INTERNAL_[A-Z_]+$/.test(id);
}
