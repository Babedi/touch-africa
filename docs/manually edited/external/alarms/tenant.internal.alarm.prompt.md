Build the `tenantInternalAlarm` module with a placement of `external` so you can reproduce the pattern for any `{moduleName}` with any placement of `{modulePlacement}`. Make sure filed and folder names use the dot notation version(with only small letters) of {moduleName} if {moduleName} is more than one word.
...
2.2. The `id` is generated server-side using a pattern like `ALARM${Date.now()}`.

3. Zod validation schema for `{json}`:

- `internalAlarmsMenuItem*`, must only contain alarm ids from `/services/{neighbourGuardService}/tenants/{tenantId}/internalAlarmsList` of the same tenant
- each alarm within `internalAlarmsMenuItem*` can only appear once
- each alarm can appear across any of the other `internalAlarmsMenuItem*`

4. Firestore path conventions:

- path: `/services/{neighbourGuardService}/tenants/{tenantId}/internalAlarms/{internalAlarmsMenuItem*}`
- where \* is one of:
  "internalAlarmsMenuItem1",
  "internalAlarmsMenuItem2",
  "internalAlarmsMenuItem3",
...
- Read: `internalRootAdmin`,`internalSuperAdmin`,`internalAlarmsManager`,`externalRootAdmin`,`externalSuperAdmin`,`externalAlarmsManager`
- Write: `internalRootAdmin`,`internalSuperAdmin`,`internalAlarmsManager`,`externalRootAdmin`,`externalSuperAdmin`,`externalAlarmsManager`
...
- GET `{modulePlacement}/{moduleName}/{internalAlarmsMenuItem*}/:alarmId` — get by ID, an alarm object from the list of alarms
- GET `{modulePlacement}/{moduleName}/{internalAlarmsMenuItem*}/list` — gets all `internalAlarmsMenuItem*` alarm objects referenced by this list from
- GET `{modulePlacement}/{moduleName}/list` — gets all regardless of `internalAlarmsMenuItem*`
- PUT `{modulePlacement}/{moduleName}/{internalAlarmsMenuItem*}/:alarmId` — update by ID
- DELETE `{modulePlacement}/{moduleName}/{internalAlarmsMenuItem*}/:alarmId` — delete by ID
