Build the `tenantExternalAlarm` module with a placement of `external` so you can reproduce the pattern for any `{moduleName}` with any placement of `{modulePlacement}`. Make sure filed and folder names use the dot notation version(with only small letters) of {moduleName} if {moduleName} is more than one word.
...
- `externalAlarmsMenuItem*`, must only contain alarm ids from `/services/{neighbourGuardService}/tenants/{tenantId}/externalAlarmsList` of the same tenant
- each alarm within `externalAlarmsMenuItem*` can only appear once
- each alarm can appear across any of the other `externalAlarmsMenuItem*`

4. Firestore path conventions:

- path: `/services/{neighbourGuardService}/tenants/{tenantId}/externalAlarms/{externalAlarmsMenuItem*}`
- where \* is one of:
  "externalAlarmsMenuItem1",
  "externalAlarmsMenuItem2",
  "externalAlarmsMenuItem3",
...
- GET `{modulePlacement}/{moduleName}/{externalAlarmsMenuItem*}/:alarmId` — get by ID, an alarm object from the list of alarms
- GET `{modulePlacement}/{moduleName}/{externalAlarmsMenuItem*}/list` — gets all `externalAlarmsMenuItem*` alarm objects referenced by this list from
- GET `{modulePlacement}/{moduleName}/list` — gets all regardless of `externalAlarmsMenuItem*`
- PUT `{modulePlacement}/{moduleName}/{externalAlarmsMenuItem*}/:alarmId` — update by ID
- DELETE `{modulePlacement}/{moduleName}/{externalAlarmsMenuItem*}/:alarmId` — delete by ID
