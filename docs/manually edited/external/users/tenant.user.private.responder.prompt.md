Build the `tenantUserPrivateResponders` module with a placement of `external` so you can reproduce the pattern for any `{moduleName}` with any placement of `{modulePlacement}`. Make sure filed and folder names use the dot notation version(with only small letters) of {moduleName} if {moduleName} is more than one word.
...
2.2. The `id` is generated server-side using a pattern like `PRVTR${Date.now()}`.
...
- For features Document `services/neighbourGuardService/tenants/{tenantId}/users/{userId}/privateResponders/{privateResponderId}`
