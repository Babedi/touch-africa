Build the `tenantUser` module with a placement of `external` so you can reproduce the pattern for any `{moduleName}` with any placement of `{modulePlacement}`. Make sure filed and folder names use the dot notation version(with only small letters) of {moduleName} if {moduleName} is more than one word.
...
2.2. The `id` is generated server-side using a pattern like `USER${Date.now()}`.

3. Zod validation schema for `{json}`:

- `title`: option from firestore path services/neighbourGuardService field lookups.titlePrefixes
- `names`: string (3-50)
- `surname`: string (3-50)
- `subAddress.streetOrFloor`: string (3-50)
- `subAddress.unit`: string (1-10)
- `activationDetails.phoneNumber`: format +27000000000
- `activationDetails.pin`: digit of length 4
- `activationDetails.preferredMenuLanguage`: option from firestore path services/neighbourGuardService field lookups.languages
- `activationDetails.isATester`: boolean and initially false
- `account.isActive.value`: boolean and initially true

4. Firestore path conventions:

- For features Document `services/neighbourGuardService/tenants/{tenantId}/users/{userId}`
...
- Tenant user is always posted to a path with a valid tenant id

6. Routes Authorization pattern (recommended):

- Read: `internalRootAdmin`,`internalSuperAdmin`,`internalTenantManager`,`externalRootAdmin`,`externalSuperAdmin`,`externalTenantManager`, `externalTenantUserManager`,`externalTenantUser`
- Write: `internalRootAdmin`,`internalSuperAdmin`,`internalTenantManager`,`externalRootAdmin`,`externalSuperAdmin`, `externalTenantManager`, `externalTenantUserManager`,`externalTenantUser`
...
Generate a externalSuperAdmin token and test against a dev server:
...
```cmd
node utilities\make-token.mjs devsecret externalSuperAdmin ADMIN_TEST > token.txt
```
