Build the `tenant` module with a placement of `external` so you can reproduce the pattern for any `{moduleName}` with any placement of `{modulePlacement}`. Make sure filed and folder names use the dot notation version(with only small letters) of {moduleName} if {moduleName} is more than one word.
...
```json
{
  ...
  "activationContextMenu": {
    ...
    "tswana": {
      "menuItem1": "Botshelo mo Kotsing",
      "menuItem2": "Thepa e mo Kotsing",
      "menuItem3": "Tšotlhe di mo Kotsing",
      "menuItem4": "",
      "menuItem5": ""
    },
    "venda": {
      "menuItem1": "Vhutshilo Vhu Khomboni",
      "menuItem2": "Pfuma Yo Khomboni",
      "menuItem3": "Zwoṱhe Zwo Khomboni",
      "menuItem4": "",
      "menuItem5": ""
    },
    "tsonga": {
      "menuItem1": "Vutomi Bya Eka Khombo",
      "menuItem2": "Ndhawu Yi Eka Khombo",
      "menuItem3": "Hinkwaswo Swi Eka Khombo",
      "menuItem4": "",
      "menuItem5": ""
    },
    "swati": {
      "menuItem1": "Kuphila Kuse Engozini",
      "menuItem2": "Ifa Lise Engozini",
      "menuItem3": "Konkhe Kuse Engozini",
      "menuItem4": "",
      "menuItem5": ""
    },
    "northernSotho": {
    ...
    "ndebele": {
      "menuItem1": "Umpilo Use Engozini",
      "menuItem2": "Ifa Lise Engozini",
      "menuItem3": "Konke Kuse Engozini",
      "menuItem4": "",
      "menuItem5": ""
    }
  },
...
2.2. The `id` is generated server-side using a pattern like `TNNT${Date.now()}`.

3. Zod validation schema for `{json}`:

- `ussdRefId`: must be incremented by 1 every time an new tenant is added on the server side
- `activationResponseBlockName`: must be unique
- `activationResponseBlockName`, `address.locality`,
  `activationContextMenu.*.menuItem1`, `activationContextMenu.*.menuItem2`,`activationContextMenu.*.menuItem3`: strings (3–50)
- `activationContextMenu.*.menuItem4`,`activationContextMenu.*.menuItem5`: strings (3–50), optional
- `activationContextMenu.*.menuItem4`, `activationContextMenu.*.menuItem4`: strings (0–50)
- `address.province`: option from lookup table in firestore path `services/{neighbourGuardService}.lookups.{provinces}`
- `address.country`: always "South Africa",
- `address.postalCode`: digits (4-5)
- `account.isActive.value`: initially true

4. Firestore path conventions:

- path: `/services/{neighbourGuardService}/tenants/{tenantId}`
...
- Read: `internalRootAdmin`,`internalSuperAdmin`,`internalAdmin`,`internalTenantManager`,`externalRootAdmin`,`externalSuperAdmin`, `externalAdmin`,`externalTenantUser`
- Write: `internalRootAdmin`,`internalSuperAdmin`,`internalTenantManager`,`externalRootAdmin`,`externalSuperAdmin`,`externalTenantManager`
