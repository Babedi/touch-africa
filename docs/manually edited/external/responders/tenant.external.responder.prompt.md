Build the `tenantExternalResponder` module with a placement of `external` so you can reproduce the pattern for any `{moduleName}` with any placement of `{modulePlacement}`. Make sure filed and folder names use the dot notation version(with only small letters) of {moduleName} if {moduleName} is more than one word.
...
```json
{
  "externalRespondersMenuItem1": [],
  "externalRespondersMenuItem2": [],
  "externalRespondersMenuItem3": [],
  "externalRespondersMenuItem4": [],
  "externalRespondersMenuItem5": []
}
```

2.2. The `id` is generated server-side using a pattern like `ERSPNDR${Date.now()}`.

3. Zod validation schema for `{json}`:

- `externalRespondersMenuItem*`, must only contain Responder ids from `/services/{neighbourGuardService}/tenants/{tenantId}/externalRespondersList` of the same tenant
- each Responder within `externalRespondersMenuItem*` can only appear once
- each Responder can appear across any of the other `externalRespondersMenuItem*`

4. Firestore path conventions:

- path: `/services/{neighbourGuardService}/tenants/{tenantId}/externalResponders/{externalRespondersMenuItem*}`
- where \* is one of:
  "externalRespondersMenuItem1",
  "externalRespondersMenuItem2",
  "externalRespondersMenuItem3",
...
- Read: `internalRootAdmin`,`internalSuperAdmin`,`internalRespondersManager`,`externalRootAdmin`,`externalSuperAdmin`,`externalRespondersManager`
- Write: `internalRootAdmin`,`internalSuperAdmin`,`internalRespondersManager`,`externalRootAdmin`,`externalSuperAdmin`,`externalRespondersManager`
...
- GET `{modulePlacement}/{moduleName}/{externalRespondersMenuItem*}/:ResponderId` — get by ID, an Responder object from the list of Responders
- GET `{modulePlacement}/{moduleName}/{externalRespondersMenuItem*}/list` — gets all `externalRespondersMenuItem*` Responder objects referenced by this list from
- GET `{modulePlacement}/{moduleName}/list` — gets all regardless of `externalRespondersMenuItem*`
- PUT `{modulePlacement}/{moduleName}/{externalRespondersMenuItem*}/:ResponderId` — update by ID
- DELETE `{modulePlacement}/{moduleName}/{externalRespondersMenuItem*}/:ResponderId` — delete by ID
