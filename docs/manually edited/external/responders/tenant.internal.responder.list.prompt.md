Build the `tenantInternalResponderList` module with a placement of `external` so you can reproduce the pattern for any `{moduleName}` with any placement of `{modulePlacement}`. Make sure filed and folder names use the dot notation version(with only small letters) of {moduleName} if {moduleName} is more than one word.
...
{
  "type": "human/entity",
  "name": "Sandton Metro Police Emergency Unit",
  "description": "",
...
2.2. The `id` is generated server-side using a pattern like `RSPNDR${Date.now()}`.

3. Zod validation schema for `{json}`:

- `type`: option from firestore path services/neighbourGuardService field lookups.responderTypes
- `name`: string (3-200)
- `phoneNumber`: format +27000000000
- `channel`: option from firestore path services/neighbourGuardService field lookups.communicationChannels
- `customMessage`: string (0-200)

4. Firestore path conventions:

- path: `/services/{neighbourGuardService}/tenants/{tenantId}/internalRespondersList/{responderId}`
...
- PUT `{modulePlacement}/{moduleName}/activate/:id` — activate by ID
- PUT `{modulePlacement}/{moduleName}/deactivate/:id` — deactivate by ID
