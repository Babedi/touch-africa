Build the `tenantInternalResponder` module with a placement of `external` so you can reproduce the pattern for any `{moduleName}` with any placement of `{modulePlacement}`. Make sure filed and folder names use the dot notation version(with only small letters) of {moduleName} if {moduleName} is more than one word.
...
```json
{
  "internalRespondersMenuItem1": [],
  "internalRespondersMenuItem2": [],
  "internalRespondersMenuItem3": [],
  "internalRespondersMenuItem4": [],
  "internalRespondersMenuItem5": []
}
```

2.2. The `id` is generated server-side using a pattern like `IRSPNDR${Date.now()}`.

3. Zod validation schema for `{json}`:

- `internalRespondersMenuItem*`, must only contain Responder ids from `/services/{neighbourGuardService}/tenants/{tenantId}/internalRespondersList` of the same tenant
- each Responder within `internalRespondersMenuItem*` can only appear once
- each Responder can appear across any of the other `internalRespondersMenuItem*`

4. Firestore path conventions:

- path: `/services/{neighbourGuardService}/tenants/{tenantId}/internalResponders/{internalRespondersMenuItem*}`
- where \* is one of:
  "internalRespondersMenuItem1",
  "internalRespondersMenuItem2",
  "internalRespondersMenuItem3",
...
- Read: `internalRootAdmin`,`internalSuperAdmin`,`internalRespondersManager`,`internalRootAdmin`,`internalSuperAdmin`,`internalRespondersManager`
- Write: `internalRootAdmin`,`internalSuperAdmin`,`internalRespondersManager`,`internalRootAdmin`,`internalSuperAdmin`,`internalRespondersManager`
...
- GET `{modulePlacement}/{moduleName}/{internalRespondersMenuItem*}/:ResponderId` — get by ID, an Responder object from the list of Responders
- GET `{modulePlacement}/{moduleName}/{internalRespondersMenuItem*}/list` — gets all `internalRespondersMenuItem*` Responder objects referenced by this list from
- GET `{modulePlacement}/{moduleName}/list` — gets all regardless of `internalRespondersMenuItem*`
- PUT `{modulePlacement}/{moduleName}/{internalRespondersMenuItem*}/:ResponderId` — update by ID
- DELETE `{modulePlacement}/{moduleName}/{internalRespondersMenuItem*}/:ResponderId` — delete by ID
