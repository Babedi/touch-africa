Build the `serviceInfo` module with a placement of `general` so you can reproduce the pattern for any `{moduleName}` with any placement of `{modulePlacement}`. Make sure filed and folder names use the dot notation version(with only small letters) of {moduleName} if {moduleName} is more than one word.
...
```json
{
  ...
  "descriptions": [
    ...
    "This service turns every mobile into an emergency response device, allowing users to send emergency alerts to their contacts and local responders.",
    "This service is designed to enhance personal safety by enabling quick emergency alerts.",
    "It allows users to send alerts via SMS, WhatsApp, and calls to designated responders.",
    "The service is integrated with a tenant registry for efficient emergency response management."
  ],

  "features": [
    {
      "title": "Emergency Communication",
      "text": "Emergency alerts via SMS, WhatsApp, and calls"
    },
    {
      "title": "Flexible Response Network",
      "text": "Customizable responder lists"
    },
    {
      "title": "Tenant Integration",
      "text": "Integration with tenant registry for efficient response"
    },
    {
      "title": "User Experience",
      "text": "User-friendly interface for quick access"
    }
  ],

  "emphasizes": [
    "tenant response and neighbour accountability.",
    "Appeals to CPF groups, estates, and tenant-watch setups.",
    "No existing use in SA panic button market (unlike Vigilant and Buzzer)",
    "turn every cell phone whether smart or not, into a panic button"
  ],

  "taglines": [
    "When your circle become a life saver.",
    "Your personal safety network at your fingertips.",
    "Empowering tenants to respond to emergencies."
  ],

  "communicationChannels": {
    "support": {
      "number": "+27635941451",
      "email": "support@neighbourguard.com"
    },
    "finance": {
      "number": "+27635941451",
      "email": "finance@neighbourguard.com"
    },
    "general": {
      "number": "+27635941451",
      "email": "general@neighbourguard.com"
    },
    "service_request": {
      "number": "+27635941451",
    ...
    "careers": {
      "number": "+27635941451",
      "email": "careers@neighbourguard.com"
    },
    "complaints": {
      "number": "+27635941451",
    ...
    "sales": {
      "number": "+27635941451",
      "email": "sales@neighbourguard.com"
    }
  },
...
- `logo`: create once and never changes
- `name`: created once and never changes
- `version`: format dd.ddd.dddd
- `initialized`: firestore date time, never changes
- `features.[*].title`: string (3–25)
- `features.[*].text`: string (50–200)
- `emphasizes`: array of strings with length (25-200)
- `public_descriptions`: array of strings with length (25-200)
- `taglines`: array of strings (15–50)
- `communication.*.for`: strings (3–25)
- `communication.*.number`: +27000000000 phone number format
- `communication.*.email`: valid email format
- `social.*.url`: valid URL format
- `social.*.icon`: strings (3–25)

4. Firestore path conventions:

- path: `services/{neighbourGuardService}/`
...
- Read: anyone
- Write: `internalRootAdmin`,`internalSuperAdmin`, `internalServiceIdentityManager`
...
- GET `{modulePlacement}/{moduleName}/` — get
- PUT `{modulePlacement}/{moduleName}/` — update
...
- Aggregate changes (e.g., append version change history)
