Build the `serviceRequest` module with a placement of `internal` so you can reproduce the pattern for any `{moduleName}` with any placement of `{modulePlacement}`. Make sure filed and folder names use the dot notation version(with only small letters) of {moduleName} if {moduleName} is more than one word.
...
```json
{
  ...
  "message": "I need to know how can I get a hold of you and arrange to purchase",
...
2.2. The `id` is generated server-side using a pattern like `SVCR${Date.now()}`.

3. Zod validation schema for `{json}`:

- `created.when`: string of date and time
- `title`: option from firestore path services/neighbourGuardService field lookups.titlePrefixes
- `names`: string (3-50)
- `surname`: string (3-50)
- `company`: string (3-50)
- `role`: string (3-50)
- `typeOfUser`: option from firestore path services/neighbourGuardService field lookups.typeOfServiceRequestUser
- `messageRelationTo`: option from firestore path services/neighbourGuardService field lookups.serviceRequestMessageRelatedTo
- `message`: string (3-200)
- `contactInfo.phone_number`: format +27000000000 Or 0000000000
- `contactInfo.email`: should be a valid email
- `processing.status`: must be set on the server side and should be an option of one of the options from firestore path services/neighbourGuardService field lookups.serviceRequestProcessingStatus

4. Firestore path conventions:

- path: `/services/{neighbourGuardService}/serviceRequests/{serviceRequestId}`
...
- The write route must be rate limited and only allow 150 quests per minute all over

6. Routes Authorization pattern (recommended):

- Read: `internalSuperAdmin`,`internalServiceRequestsManager`
- Write: everyone
