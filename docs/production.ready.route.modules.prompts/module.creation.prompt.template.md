...
```
MODULE_NAME: [e.g., "tenantUser", "serviceRequest", "externalAlarm"]
MODULE_TYPE: [e.g., "external", "internal", "general"]
ROUTE_PREFIX: [e.g., "/external/tenantUser", "/internal/admin", "/general/serviceInfo"]
COLLECTION_NAME: [e.g., "tenantUsers", "serviceRequests", "externalAlarms"]
ID_PREFIX: [e.g., "USR", "SVCR", "ALARM"]
REQUIRES_TENANT_HEADER: [true/false - does this module need x-tenant-id header?]
REQUIRES_AUTH: [true/false - does this module require JWT authentication?]
HAS_LOGIN_SYSTEM: [true/false - does this module need login/logout authentication routes?]
HAS_AVAILABILITY_STATUS: [true/false - does this module track availability/online status?]
HAS_SOFT_DELETE: [true/false - does this module support soft delete with deletedAt timestamps?]
HAS_AUDIT_TRAIL: [true/false - does this module need comprehensive audit logging?]
HAS_BULK_OPERATIONS: [true/false - does this module support bulk create/update/delete operations?]
HAS_SEARCH_FILTER: [true/false - does this module need advanced search and filtering capabilities?]
HAS_PAGINATION: [true/false - does this module need pagination support for list operations?]
HAS_CACHING: [true/false - does this module need Redis caching for performance?]
HAS_FILE_UPLOAD: [true/false - does this module handle file uploads?]
HAS_REAL_TIME: [true/false - does this module need WebSocket/Server-Sent Events support?]
LOGIN_CREDENTIALS: [if HAS_LOGIN_SYSTEM=true, specify credential fields, e.g., "email, password" or "username, pin"]
READ_ROLES: [comma-separated list of roles that can read, e.g., "internalRootAdmin, internalSuperAdmin, externalAdmin"]
WRITE_ROLES: [comma-separated list of roles that can write, e.g., "internalRootAdmin, internalSuperAdmin"]
MAIN_ENTITY_FIELDS: [list the primary fields for the main entity, e.g., "serialNumber, modelDescription, accessDetails"]
VALIDATION_RULES: [describe specific validation requirements, e.g., "phone numbers must be in +27 format, pins must be 4 digits"]
SAMPLE_DATA_VALUES: [provide realistic sample values for testing]
MENU_INTEGRATION: [true/false - does this module integrate with tenant menu systems?]
RELATIONSHIP_MODULES: [list any modules this connects to, e.g., "tenant", "alarm"]
PERFORMANCE_REQUIREMENTS: [specify any performance requirements, e.g., "sub-100ms response time", "high-throughput writes"]
BUSINESS_RULES: [describe any specific business logic requirements, e.g., "max 5 active alarms per tenant", "approval workflow required"]
```
