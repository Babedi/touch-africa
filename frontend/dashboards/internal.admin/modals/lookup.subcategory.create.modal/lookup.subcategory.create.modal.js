...
(function () {
  const containerId = "lookupSubCategoryCreateModal";
  const htmlPath =
    "/frontend/dashboards/internal.admin/modals/lookup.subcategory.create.modal/lookup.subcategory.create.modal.html";
  ...
  function validateAll(form, root) {
    let ok = true;
    ["subcategory", "description"].forEach((n) => {
  ...
  function attachRealtimeValidation(root) {
    const form = qs("#lookupSubCategoryCreateForm", root);
  ...
  function ensureOverlay() {
    ...
    if (!el) {
      ...
      el.id = containerId;
      el.className =
        "modal-overlay modal-lg modal-lookup-subcategory modal-lookup-subcategory-create";
  ...
  async function ensureContainer() {
    ...
    if (!c.dataset.loaded) {
      try {
        ...
        console.error("[LookupSubCategoryCreate] html load failed", e);
        c.innerHTML =
          '<div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h2>Create Lookup Sub-Category</h2><button class="modal-close" aria-label="Close">&times;</button></div><div class="modal-body"><p>Unable to load create modal.</p><div class="form-actions"><button type="button" class="btn btn-secondary" data-action="cancel">Close</button></div></div></div></div>';
      }
  ...
  async function submitHandler(e) {
    ...
    try {
      ...
      await api.lookupSubCategories.create(payload);
      notify("success", "Sub-category created");
      close();
      window.reloadLookupsList?.();
    } catch (err) {
      console.error("[LookupSubCategoryCreate] create failed", err);
  ...
  async function open() {
    ...
    const form = qs("#lookupSubCategoryCreateForm", c);
    if (form) form.reset();
  }
  function close() {
  ...
  window.openLookupSubCategoryCreateModal = open;
})();
